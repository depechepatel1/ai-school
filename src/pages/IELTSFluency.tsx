/**
 * IELTS Shadowing Fluency Screen
 * 
 * Uses ielts/shadowing-fluency.json (from curriculum bucket, same as ielts-shadowing.json).
 * Presents model answer chunks one at a time with karaoke text.
 * Timer: 10-minute countdown from timer_settings.
 */
...
export default function IELTSFluency() {
  ...
  // Derive info panel data
  const sectionMap: Record<string, string> = {
    model_answer: "Model Answer", part_2: "Part 2", part_3: "Part 3",
  };
  ...
  // Check if at last chunk in current answer (simple heuristic: next chunk has different question)
  ...
  const handleRepeatAnswer = () => {
    // Go back to first chunk of current answer
    const currentQ = shadowCurriculum.currentChunk?.question_text;
    ...
    <PageShell fullWidth loopVideos={VIDEO_1_STACK} hideFooter>
      <div className="relative w-full h-full text-white font-outfit select-none animate-fade-in-up">
        ...
        <div className="absolute top-4 left-4 z-[300] flex items-center gap-2">
          ...
          <span className="px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase tracking-[0.12em] backdrop-blur-2xl bg-cyan-500/20 border border-cyan-500/30 text-cyan-300">
            IELTS · Homework · Shadowing Fluency
          </span>
        </div>
              ...
              <div className="bg-black/80 backdrop-blur-2xl border border-white/[0.12] rounded-3xl p-8 max-w-sm w-full mx-4 shadow-[0_0_60px_rgba(0,0,0,0.5)]">
                ...
                <div className="flex flex-col gap-2.5">
                  <button
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-amber-500/20 border border-amber-500/30 text-amber-200 text-sm font-bold hover:bg-amber-500/30 transition-all flex items-center gap-2 justify-center"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-cyan-500/20 border border-cyan-500/30 text-cyan-200 text-sm font-bold hover:bg-cyan-500/30 transition-all flex items-center gap-2 justify-center"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-white/[0.06] border border-white/[0.08] text-white/60 text-sm font-bold hover:bg-white/10 transition-all flex items-center gap-2 justify-center"
                  >
