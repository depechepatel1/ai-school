            ...
            >
              ...
              <motion.div variants={fadeUp} className="flex items-center gap-3 mb-5">
                  ...
                  {(["ielts", "igcse"] as const).map((ct) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all ${
                        newCourseType === ct
                          ? "bg-teal-500/15 border border-teal-400/20 text-teal-300"
                ...
                <button
                  ...
                  className="px-4 h-11 rounded-xl bg-teal-600 text-white text-xs font-semibold hover:bg-teal-500 disabled:opacity-40 transition-all flex items-center gap-1.5"
                >
              ...
              <div className="flex-1 overflow-y-auto scrollbar-hide mb-5">
                {classesLoading ? (
                  ...
                  <motion.div variants={fadeUp} className="text-center py-12">
                    <div className="w-14 h-14 rounded-2xl bg-white/[0.03] border border-white/[0.06] flex items-center justify-center mx-auto mb-3 hover:scale-105 transition-transform">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                    {classes.map((c) => (
                      <motion.div
                        ...
                        className="p-4 rounded-2xl bg-white/[0.02] border border-white/[0.06] hover:bg-white/[0.04] hover:border-white/10 transition-all space-y-2.5 cursor-pointer group"
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-white/[0.04] border border-white/[0.06] text-[10px] font-semibold text-gray-400">
                        ...
                        <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                          <code className="flex-1 px-2.5 py-1.5 bg-white/[0.04] rounded-lg text-[10px] text-teal-300 font-mono tracking-wider">{c.join_code}</code>
                          <button
                            onClick={() => copyCode(c.join_code)}
                            className="p-1.5 rounded-lg text-gray-500 hover:text-teal-300 hover:bg-teal-500/10 transition-all"
                          >
              ...
              <div className="flex gap-3 mt-auto">
                {[
                  ...
                  <motion.div
                    key={i}
                    variants={fadeUp}
                    className="flex-1 flex items-center gap-3 px-4 py-3 rounded-2xl bg-white/[0.02] border border-white/[0.06] group cursor-default"
                  >
                    <div className="w-9 h-9 rounded-lg bg-teal-500/[0.08] border border-teal-400/10 flex items-center justify-center text-teal-400 shrink-0">
                      {item.icon}
                    </div>
                    <h3 className="text-xs font-semibold text-gray-400 flex-1">{item.title}</h3>
                    <span className="px-2 py-0.5 rounded-full bg-emerald-500/15 border border-emerald-400/20 text-[10px] font-bold uppercase tracking-widest text-emerald-300">{item.tag}</span>
                  </motion.div>
                ))}
              </div>
            </motion.div>
