...
export default function HomeworkModal({ isOpen, onClose }: HomeworkModalProps) {
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-fade-in-up">
      <div className="bg-gray-900 border border-teal-500/30 p-6 rounded-3xl max-w-sm w-full m-4 shadow-2xl flex flex-col h-[60%]">
        <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2 flex-shrink-0">
          <BookOpen className="text-teal-400" /> Homework
        </h3>
        <div className="space-y-3 flex-1 overflow-y-auto scrollbar-hide pr-2">
          {[
            ...
            { title: "Speaking Part 2 Recording", due: "No Deadline", color: "text-teal-400" },
            { title: "Vocabulary Review: Health", due: "Recommended", color: "text-teal-400" },
          ].map((hw, i) => (
        ...
        <button onClick={onClose} className="mt-4 w-full py-3 bg-teal-600 rounded-xl text-white font-bold hover:bg-teal-500 transition-colors flex-shrink-0">Close</button>
      </div>
    </div>
  );
}
