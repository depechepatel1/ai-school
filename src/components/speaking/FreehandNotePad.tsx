    ...
    <div className="absolute top-[150px] right-5 w-[280px] h-[220px] bg-white/[0.03] backdrop-blur-[40px] border border-white/10 rounded-2xl flex flex-col z-[200] animate-fade-in overflow-hidden">
      <div className="p-3 border-b border-white/10 flex items-center justify-between bg-white/5">
        ...
        <div className="flex gap-2 items-center">
          <button onClick={() => setMode("type")} className={`p-1.5 rounded transition-colors ${mode === "type" ? "bg-white/20 text-white" : "hover:bg-white/10 text-white/60"}`}><Keyboard className="w-4 h-4" /></button>
          <button onClick={() => setMode("draw")} className={`p-1.5 rounded transition-colors ${mode === "draw" ? "bg-white/20 text-white" : "hover:bg-white/10 text-white/60"}`}><PenTool className="w-4 h-4" /></button>
          <button onClick={clearCanvas} className="p-1.5 hover:bg-white/10 rounded text-white/60"><Trash className="w-4 h-4" /></button>
        </div>
      </div>
      <div ref={wrapperRef} className="flex-1 relative bg-white/5 overflow-hidden">
        {mode === "draw" ? (
          ...
          <textarea value={typedText} onChange={(e) => setTypedText(e.target.value)}
            className="w-full h-full bg-transparent p-4 text-white font-mono text-sm resize-none focus:outline-none placeholder-white/20" placeholder="Type your notes here..." />
        )}
      </div>
    </div>
  );
