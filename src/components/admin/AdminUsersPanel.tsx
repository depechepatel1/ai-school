          ...
          <AnimatePresence>
            {confirmDelete === u.id && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex items-center gap-2 p-2 rounded-lg bg-red-500/10 border border-red-500/20">
                ...
                <button disabled={busy} onClick={() => handleDelete(u.id)} className="px-2 py-1 rounded text-xs font-bold bg-red-500/20 text-red-300 hover:bg-red-500/30 transition-all disabled:opacity-50">{busy ? "…" : "Delete"}</button>
